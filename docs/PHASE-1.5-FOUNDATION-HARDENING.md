...
- **Anchor math unified** and moved to [src/shared/anchoring.ts](../src/shared/anchoring.ts) — used by both DB (boundary repair after every step) and renderer (building anchors from selections). Previous duplicate lived in [src/renderer/editor/workbenchUtils.ts](../src/renderer/editor/workbenchUtils.ts).
...
- **Schema migrations** in [src/db/migrations.ts](../src/db/migrations.ts) — `user_version`-driven, ordered, transactional. The ad-hoc `CREATE TABLE IF NOT EXISTS` + `ensureColumn` code in the old [src/db/schema.ts](../src/db/schema.ts) is retained as legacy-shape helpers but is no longer how the schema evolves.
